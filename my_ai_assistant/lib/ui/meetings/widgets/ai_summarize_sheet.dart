  ...
  Future<void> _handleSummarize() async {
    ...
    try {
      ...
      if (summaryResult.isNotEmpty) {
        ...
        if (_sessions.isEmpty || _activeSessionIndex >= _sessions.length) {
          // Gracefully create a fallback session or wait for _initSessions()
          final uid = AuthService().currentUser?.uid ?? 'temp';
          ...
          ...widget.recordingTakes
              ...
              .map((take) {
                return CheckboxListTile(
                  ...
                  value: _includeTakes[take['id']?.toString() ?? ''],
                  activeColor: GlassColors.primary,
                  checkColor: GlassColors.onPrimary,
                  onChanged: (val) =>
                      setState(() => _includeTakes[take['id']?.toString() ?? ''] = val ?? false),
                );
              }),
