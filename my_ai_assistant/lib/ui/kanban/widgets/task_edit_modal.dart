...
class _TaskEditModalState extends State<TaskEditModal> {
  ...
  void initState() {
    ...
    if (widget.existingTask != null) {
      ...
      _status =
          ...
          (widget.board.columns.isNotEmpty
              ? widget.board.columns.first
              : 'todo');
    }
  ...
  Future<void> _loadCoverPreference() async {
    final prefs = await SharedPreferences.getInstance();
    final saved = prefs.getBool(_coverPrefKey);
    if (!mounted || saved == null) return;
    setState(() => _isCoverExpanded = saved);
  }

  Future<void> _persistCoverPreference(bool value) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(_coverPrefKey, value);
  }

  Future<void> _setCoverExpanded(bool value) async {
    if (_isCoverExpanded == value) return;
    setState(() => _isCoverExpanded = value);
    await _persistCoverPreference(value);
  }
  ...
  Future<void> _pickAndUploadImage() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('อัปโหลดไฟล์ล้มเหลว: $e')));
    } finally {
      if (mounted) setState(() => _isUploading = false);
    }
  }

  Future<void> _generateAndSaveDescriptionInBackground(
    String imageId,
    List<int> bytes,
    String fileName,
  ) async {
    ...
    try {
      final aiDescription = await ApiCloudflare.generateAiDescription(
        bytes,
        mimeType,
      );
      if (mounted) {
  ...
  Widget _buildCoverBanner(TaskImage image) {
    return SizedBox(
      height: 220,
      child: Stack(
        children: [
          Positioned.fill(
            child: GestureDetector(
              ...
              child: ClipRRect(
                ...
                child: Image.network(
                  EnvConfig.sanitizeUrl(image.url),
                  fit: BoxFit.cover,
  ...
  Widget _buildAssetRow(TaskImage img) {
    ...
    if (!_assetNameControllers.containsKey(img.id)) {
      final displayName = img.name.isNotEmpty
          ? img.name
          : (img.r2Key.isNotEmpty
                ? img.r2Key.split('/').last
          ...
          if (_allowStructuralEditing)
            IconButton(
              ...
              onPressed: () {
                setState(() {
                  _images = _images
                      .map((i) => i.copyWith(isCover: i.id == img.id))
                      .toList();
                });
                _autoSaveTask();
              },
  ...
  void _showFullImage(TaskImage image) {
    final description = image.aiDescription.trim();
    showDialog(
      context: context,
      builder: (context) {
        final isDesktop = MediaQuery.of(context).size.width > 900;
            ...
            child: isDesktop
                ? Row(
                    children: [
                      Expanded(
                        flex: 7,
                        child: _buildImagePreviewPane(image, isDesktop: true),
                      ),
                      Container(
                        width: 320,
                        decoration: BoxDecoration(
                          border: Border(
                            left: BorderSide(color: GlassColors.ghostBorder),
                          ),
                        ),
                        child: _buildImageDescriptionPane(
                          image,
                ...
                : Column(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      _buildImagePreviewPane(image, isDesktop: false),
                      _buildImageDescriptionPane(
                        image,
                        description: description,
                      ),
                    ],
  ...
  Widget _buildImagePreviewPane(TaskImage image, {required bool isDesktop}) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        Padding(
          ...
          child: Row(
            children: [
              Expanded(
                child: Text(
                  image.name.isNotEmpty ? image.name : 'IMAGE PREVIEW',
                  maxLines: 1,
  ...
  Widget _buildImageDescriptionPane(
    TaskImage image, {
        ...
        children: [
          ...
          Text(
            description.isNotEmpty
                ? description
                : 'ยังไม่มีคำอธิบายของรูปนี้ในตอนนี้',
            style: GlassText.bodyMD().copyWith(
  ...
  Widget _buildTitleSection({required bool isDesktop}) {
    ...
    if (widget.existingTask == null) {
      return Row(children: [titleField]);
    }
    ...
    return Row(
      ...
      children: [
        ValueListenableBuilder<TaskModel>(
          valueListenable:
              _taskNotifier ?? ValueNotifier<TaskModel>(widget.existingTask!),
                ...
                Row(
                  ...
                  children: [
                    ...
                    TextButton.icon(
                      ...
                      onPressed: () =>
                          _createNewBoardLabel(currentBoard, setModalState),
                    ),
                  ],
                ),
                const SizedBox(height: 16),
        ...
        builder: (context, setDialogState) {
          return AlertDialog(
            ...
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(context),
                child: Text('CANCEL', style: GlassText.label()),
              ),
              TextButton(
                onPressed: () async {
                  if (controller.text.isNotEmpty) {
                    await context.read<StateBoards>().addLabel(
                      currentBoard,
                      controller.text,
                      selectedColor,
                    );
                ...
                child: Text(
                  'CREATE',
                  style: GlassText.label().copyWith(color: GlassColors.gold),
                ),
              ),
            ],
          );
            ...
            child: Column(
              ...
              children: [
                ...
                ..._availableMembers.entries.map((entry) {
                  ...
                  final role =
                      currentBoard.memberRoles[entry.key] ?? 'Operative';
                      ...
                      children: [
                        CircleAvatar(
                          ...
                          onForegroundImageError: photo.isNotEmpty
                              ...
                              : null,
                          backgroundColor: GlassColors.getMemberColor(
                            entry.key,
                          ).withOpacity(0.2),
                          child: Text(
                            ...
                            style: GlassText.labelSM().copyWith(
                              fontSize: 10,
                              color: GlassColors.getMemberColor(entry.key),
                            ),
                          ),
                        ),
                        const SizedBox(width: 12),
  ...
  Widget _buildTaskChatMessageBubble(ChatMessage msg) {
    ...
    return Align(
      ...
      child: Container(
        ...
        child: Text(
          msg.text,
          style: GlassText.bodyMD().copyWith(color: GlassColors.onSurface),
        ),
      ),
    );
  }
    ...
    return Row(
      children: [
        ...
        IconButton(
          ...
          onPressed: _isReadOnly ? null : () => _sendTaskChatMessage(chatState),
        ),
      ],
    );
  }
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            color: isMe
                                ...
                                : GlassColors.onSurface.withOpacity(0.03),
        ...
        Expanded(
          child: Container(
            ...
            child: ImeSafeTextField(
              ...
              onSubmitted: (_) =>
                  _handleAddComment(currentUid, currentUserName),
            ),
          ),
        ),
        const SizedBox(width: 12),
    ...
    if (isDesktop) {
      ...
      return Column(
        ...
        children: [
          ...
          if (!_isReadOnly) ...[const SizedBox(height: 24), inputWidget],
        ],
      );
    }
  }
